...
<h3 class="fn"><a name="winEventFilter" />(bool, int&#160;<i>result</i>) QCoreApplication.winEventFilter (<i>self</i>, <a href="msg.html">MSG</a>&#160;<i>message</i>)</h3><p>The message procedure calls this function for every message
received. Reimplement this function if you want to process window
messages <i>msg</i> that are not processed by Qt. If you don't want
the event to be processed by Qt, then return true and set
<i>result</i> to the value that the window procedure should return.
Otherwise return false.</p>
<p>It is only directly addressed messages that are filtered. To
handle system wide messages, such as messages from a registered hot
key, you need to install an event filter on the event dispatcher,
which is returned from <a href="qabstracteventdispatcher.html#instance">QAbstractEventDispatcher.instance</a>().</p>
